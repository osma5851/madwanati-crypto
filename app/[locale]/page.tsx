...
export default async function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--t-bg, #09090b)' }}>
      <Navbar />
      <CryptoTicker />
      <main style={{ flex: 1 }}>
        {/* Hero */}
        <section className="hero-bg" style={{ padding: '5rem 1rem 4rem', overflow: 'hidden' }}>
          <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', zIndex: 1 }}>
            ...
            <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.15, color: 'var(--t-fg, #fafafa)', marginBottom: 16, letterSpacing: '-0.02em' }}>
              {t("title")}
              <span className="gold-text" style={{ display: 'block', marginTop: 8 }}>{t("brand")}</span>
            </h1>

            <p style={{ fontSize: 'clamp(0.9rem, 1.5vw, 1.05rem)', lineHeight: 1.7, maxWidth: 560, color: 'var(--t-fg3, #71717a)', marginBottom: 40 }}>
              {t("subtitle")}
            </p>
            ...
            <div className="stats-box">
              {[
                ...
                <div key={stat.label} className="stats-item">
                  ...
                  <div style={{ fontSize: '0.7rem', fontWeight: 500, color: 'var(--t-fg4, #52525b)', marginTop: 2 }}>{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ background: 'var(--t-bg, #09090b)' }}>
